...
TapSuite[TapTestSame[GCD[2, 3, 5], 1], 
 TapComment["Plot the RefLink[GCD,paclet:ref/GCD] for a number with 1000: "], 
 TapTestSameBROKEN[HoldComplete[DiscretePlot[GCD[1000, n], {n, 100}]], 
  $Failed], TapTestSame[GCD[2, {3, 5, 8}], {1, 1, 2}], 
 TapTestSame[GCD[1/3, 2/5, 3/7], 1/105], TapTestSameBROKEN[{1/3, 2/5, 3/7}/%, 
  {35, 42, 45}], TapTestSame[GCD[-10 + 8*I, 5 - 4*I], 4 + 5*I], 
 TapComment[
  "RefLink[TraditionalForm,paclet:ref/TraditionalForm] formatting:"], 
 TapTestSameBROKEN[HoldComplete[gcd(x, y)], $Failed], 
 TapTestSameBROKEN[HoldComplete[ArrayPlot[Table[GCD[m, n], {m, 60}, 
     {n, 60}]]], $Failed], TapComment["Find the fraction of pairs of the \
first 100 numbers that are relatively prime:"], 
 TapTestSame[N[Total[Boole[Table[GCD[m, n] == 1, {m, 100}, {n, 100}]], 2]/
    10^4], 0.6087], TapComment["The result is close to 1/\\[Zeta](2):"], 
 TapTestSameBROKEN[N[1/Zeta[2]], 0.607927], 
 TapComment[
 ...
 TapTestSameBROKEN[HoldComplete[
   ArrayPlot[Mod[Table[GCD[Fibonacci[j], Fibonacci[k]], {j, 100}, {k, 100}], 
...
pairwise GCDs is related to Euler's totient function:"], 
 ...
 TapTestSame[GCD[a, GCD[b, c]] == GCD[GCD[a, b], c], True], 
 TapTestSame[GCD[a, b] == GCD[b, a], True], TapTestSameBROKEN[GCD[a] == a, 
  True], TapComment["The zero argument form is zero: "], 
 TapTestSame[GCD[], 0], TapComment[
