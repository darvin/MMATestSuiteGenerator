...
\[ScriptCapitalA]\[ScriptCapitalB]\[ScriptCapitalC] \[ReturnIndicator]"], 
 ...
 TapTestSame[Cases[{1, 2, "ab", "cd", x, y}, _String], {ab, cd}], 
...
cd"], TapComment["Create formatted text:"], 
 ...
 TapComment["Define a function that applies only to strings:"], 
 TapTestSame[f[s_String] := StringJoin[RotateLeft[Characters[s]]]; 
   {f["X"], f["ABCDEF"], f[x], f[{a, b, c}]}, 
  {X, BCDEFA, f[x], f[{a, b, c}]}], 
 TapComment["Use strings as plot markers:"], 
 TapTestSame[HoldComplete[ListPlot[{{1, 2, 3}, {3, 1, 4}}, 
    PlotMarkers -> {"A", "B"}]], $Failed], 
 TapComment["Give text in graphics:"], 
 TapTestSame[HoldComplete[Graphics[{Text["abcde", {0, -2}], Disk[{0, 0}], 
...
the long form of the character: "], TapTestSame["\[Alpha]", \[Alpha]], 
