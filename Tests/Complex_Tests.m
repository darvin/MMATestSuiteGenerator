...
exactly zero, then the result is not RefLink[Complex,paclet:ref/Complex]:"], 
 ...
 TapComment["You have to use RefLink[Re,paclet:ref/Re] and \
RefLink[Im,paclet:ref/Im] to extract parts of \
RefLink[Complex,paclet:ref/Complex] numbers:"], 
 TapTestSameBROKEN[x = 1 + 2*I ;; {Re[x], Im[x]}, {1, 2}], 
...
, TapTestSameBROKEN[1.`20. + 2.`30.*I, 1.`19. + 2.`29.301029995663985*I], 
...
pattern:"], TapTestSame[MatchQ[1 + 2*I, _Complex], True], 
 TapComment["A rule that switches real and imaginary parts:"], 
 TapTestSameBROKEN[
  rule = x_Complex -> Im[x] + I*Re[x] ;; f[1 + 2*I, 3 + 4*I, 5] /. rule, 
  f[2 + I, 4 + 3*I, 5]], TapComment["An alternate definition:"], 
 TapTestSame[f[1 + 2*I, 3 + 4*I, 5] /. Complex[r_, i_] :> Complex[i, r], 
  f[2 + I, 4 + 3*I, 5]], TapComment["Define a function over the complexes by \
using functions defined over the reals: "], 
 TapTestSameBROKEN[HoldComplete[
   cosine[x_Complex] := Cos[Re[x]]*Cosh[Im[x]] - I*Sin[Re[x]]*Sinh[Im[x]]; 
    cosine[x_Real] := Cos[x]; Plot3D[Abs[cosine[x + I*y]], {x, -3, 3}, 
     {y, -1, 1}]], $Failed], TapComment["Complexes are numbers:"], 
...
conditions: "], TapTestSame[Reduce[z*Conjugate[z] < 1, z, Complexes], 
  -1 < Re[z] < 1 && -Sqrt[1 - Re[z]^2] < Im[z] < Sqrt[1 - Re[z]^2]], 
 TapComment["Numbers entered in the form x+RefLink[I,paclet:ref/I]y only \
become RefLink[Complex,paclet:ref/Complex] numbers on evaluation:"], 
 TapTestSame[SetAttributes[f, HoldAll]; f[x_Complex] := Re[x] - Im[x]; 
   f[1 + 2*I], f[1 + 2*I]], TapTestSame[f[Evaluate[1 + 2*I]], -1], 
 TapComment["The unevaluated form is expressed in terms of \
